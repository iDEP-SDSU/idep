...
output$EDA <- renderPlot({
    ...
    legend("topright", cex=2,colnames(x), lty=rep(1,dim(x)[2]), col=myColors )	
...
output$scatterPlot <- renderPlot({
 ...
 if(0){ 
	...
	genes = genes[which(genes$upOrDown != 1),]  # remove non-DEGs
	...
	layout(
           xaxis = list(size =35,title = paste("Average expression in", samples[2] )), 
