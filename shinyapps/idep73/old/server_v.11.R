...
output$fileFormat <- renderText({
	  ...
	  Most types of common gene ids are accepted. Name samples in each column carefully as iDEP parses them to define sample groups. Replicates should be denotated by \"_Rep1\", 
...
output$selectedHeatmap <- renderPlot({
	...
	isolate({ 
	  ...
	  if(is.null(ix)) reutn(NULL); if(is.na(ix)) reutrn (NULL)
...
output$scatterPlot <- renderPlot({
 ...
 # write.csv(genes, "tem.csv")
	plot(genes$average2,genes$average1,col = c("grey45","blue", "red")[genes$upOrDown],
