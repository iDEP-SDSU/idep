...
DEG.limma <- function (x, maxP_limma=.1, minFC_limma=2){

eset = new("ExpressionSet", exprs=as.matrix(x))

 groups = colnames(x)
 groups = detectGroups( groups)
 g = sort( levels(groups),decreasing=T)
...
fit2 <- eBayes(fit2)
 ...
 topGenes =topTable(fit2, number=20,sort.by="M",p.value=maxP_limma, lfc=log(minFC_limma) )
 topGenes = topGenes[,c('logFC','adj.P.Val')] 
...
#vennDiagram(results[,1:5],circle.col=rainbow(5))
 top <- function (comp) topTable(fit2, coef=comp, number=200,sort.by="M",p.value=maxP_limma, lfc=log(minFC_limma) )[,c(1,5)]
...
readData <- reactive ({ 
    ...
    if (input$dataFileFormat == 2 ) {  # if FPKM
		...
		if( dim(x)[2] <= 10) { x <- rlog(dds, blind=FALSE) } else
    ...
    return( list(data = as.matrix(x), mean.kurtosis = mean.kurtosis, rawCounts = rawCounts) )
	 })
	})
  })	
...
output$fileFormat <- renderText({
      ...
      WT_control_Rep1, WT_control_Rep2, WT_Treatment_Rep1, WT_treatment_Rep2,
		...
		Also, avoid using \"-\" in sample names."
	})	
...
output$KmeansPromoter <- renderTable({
    if (is.null(input$file1)&& input$goButton == 0)   return(NULL)
	tem = input$selectGO3; tem = input$radioPromoterKmeans
	if( is.null(input$selectGO3 ) ) return (NULL)
...
output$geneListGO <- renderTable({
	...
	result = FindOverlap (convertedID,tem, input$selectGO2,selectOrg,1) 
	if( dim(result)[2] ==1) next;   # result could be NULL
