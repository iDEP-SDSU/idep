...
a(" demo.", href="https://idepsite.wordpress.com/demo/"),
   "Based on annotation of",a( " 69 metazoa and 42 plant genomes ",href="https://idepsite.wordpress.com/species/") ,"in Ensembl BioMart as of 11/15/2016."
    ...
    sidebarLayout(
      sidebarPanel(
	    actionButton("goButton", "Try it with demo data"),
		...
		selectInput("selectOrg", label = NULL,"Best matching species",width='100%'), 
		 ...
		 conditionalPanel("input.selectOrg == 'NEW'",
			fileInput('gmtFile', 'Choose gene set file (.gmt) to upload (<50Mb)',
                  accept = c(
                    'text/csv',
                    'text/comma-separated-values',
                    'text/tab-separated-values',
...
tableOutput('species' ),
    ...
    ,tabPanel("Heatmap",
              sidebarLayout(
                sidebarPanel(
   					sliderInput("nGenes", label = h4("Most variable genes to include in hierarchical clustering"), min = 10, max = 2000, value = 1000,step=50) 
					,downloadButton('downloadData', 'Download heatmap data')
					,br(),br()
					,actionButton("showCorrelation", "Show correlation matrix")
		 ,br(),br()
			,a("Help", href="https://idepsite.wordpress.com/heatmap/")
					),
                mainPanel(				  
