     ...
     ,tabPanel("PCA",
	     ...
	     ,tabPanel("DEGs",
              sidebarLayout(
				...
				selectInput("CountsDEGMethod", "Method to detect DEGs from read counts:", choices = list("DESeq2"= 3,"limma-voom"=2,"limma-trend"=1), selected = 1)				
				)				
				,fluidRow(
			...
			,selectInput("pathwayMethod", label = "Select method and genesets:", choices = list("GAGE" = 1,  "GSEA (preranked fgsea)" =3,"PGSEA" = 2, "PGSEA w/ all samples" =4, "ReactomePA" = 5), selected = 1) #
                ...
                mainPanel(	  
				  ...
				  plotOutput("PGSEAplotAllSamples") )	
					...
					,conditionalPanel("input.selectGO == 'KEGG'",imageOutput("KeggImage"))
					,conditionalPanel("input.selectGO != 'KEGG'",plotOutput("selectedPathwayHeatmap"))
