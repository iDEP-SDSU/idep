...
########### 
Following removed to constant.config file
#####
...
redudantGeneSetsRatio = 0.9 <- CONST_redudantGeneSetsRatio
...
`}  
 ...
 detectGroups <- LogicManager$PreProcessing$DetectGroups

# heatmap with color bar define gene groups
myheatmap2 <- Display.Manager$GetHeatmapWithGeneGroups
...
cleanGeneSet <- DB.Manager$cleanGeneSet()

readGMT <- DB.Manager$readGMT
readGMTRobust <- DB.Manager$readGMTRobust

convertId <- PreProcessing.Logic$set("public", "GetConvertID"
...
findSpeciesById <- function() # DB.Manager$set("public", "findSpeciesById",

findSpeciesByIdName <- function() # DB.Manager$set("public", "findSpeciesNameById",

# Main function. Find a query set of genes enriched with functional category
FindOverlap <- function() # DB.Manager$set("public", "findOverlap"
...
# Main function. Find a query set of genes enriched with functional category
readGeneSets <- function (converted, convertedData, GO,selectOrg, myrange) { ### DB.Manager$set("public", "QueryGeneSetsFromPathway"
} 
...
enrichmentPlot <-  # Display.Manager$GetEnrichmentPlot
...
output$genePlot <-  new : output$PreProcess_SingleGenePlot 
genePlot4Download <- Donot need anymore

output$downloadGenePlot <- new : output$download_PreProcess_PlotSingleGenes
...
plotHeatmap1 <- #output$download_Heatmap_DownloadEpsFormatPlot
	# conventional heatmap.2 plot


# interactive heatmap with plotly
output$heatmapPlotly <- #output#Heatmap_HeatmapPlotly
...
output$sampleTree <- # output$Heatmap_SampleTree
...
#   PCA
################################################################
output$listFactors <- Ctl.PCA$InitColorSelection # div_PCA_MainPlotColorSelection
	
output$listFactors2 <- Ctl.PCA$InitShapeSelection # div_PCA_MainPlotShapeSelection
...
output$PCA <- output$PCA_mainplot
...
Kmeans <- Kmeans()  #ReactiveVars$KmeansReactiveVar
...
KmeansData <- reactive({ })  ## ReactiveVars$KmeansWithGeneInfo
...
KmeansGOdata <- # ReactiveVars$KmeansGoData
output$KmeansGO <- # output$Kmeans_GO_Table

output$downloadKmeansGO <- #output$download_Kmeans_KmeansGO
	
output$enrichmentPlotKmeans <- # output$Kmeans_EnrichmentPlot


output$enrichmentPlotKmeans4Download <- # download_Kmeans_EnrichmentPlot


output$KmeansPromoter <- # output$Kmeans_Promoter_Table
