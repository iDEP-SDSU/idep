...
#             "HiSeq 2000 and HiSeq 2500 raw data from NCBI's SRA database are 
 #            aligned with Kallisto against human GRCh38 or mouse GRCm38 cDNA reference.")
         ,fluidRow( column(9),
                           #, p(align="center","(1) Select your species from the box below\n, search, 
                              #  and click on a dataset to see more information before download.") )
                    ...
                    # ,br(),br(), br(),p(align="left","   (2) Use the search bar to find the correct sample series")
                    # ,br(), p("    (3) Select the appropriate row to view a preview of the file")
                    # , br(),br(), p("   (4) Click the 'Gene-level counts' or the 'Transcript-level counts' box to download the dataset")
                    # 
                    # ,br(), 
                    # 
                    br(),
                    # 
                    tags$ol(tags$li("Select your species from the box below"),
                            tags$li( "Use the search bar to find the correct sample series"),
                            tags$li("Select the appropriate row to view a preview of the file"),
                            tags$li("Click the 'Gene-level counts' or the 'Transcript-level counts' box to download the dataset"))
                      
                    
                    
                    ,selectInput("selectedSpecies", label = NULL,"", width='25%')
         )
         ...
         ,fluidRow( column(2, h4( textOutput('selectedDataset')))
                   ...
                   ,column(3,downloadButton('downloadSearchedDataTranscript', 'Transcript-level counts') )                   
                    ,column(4, h5("Edit and upload file to ",a("iDEP", href="http://bioinformatics.sdstate.edu/idep/"), "for analysis") )                            
         )
